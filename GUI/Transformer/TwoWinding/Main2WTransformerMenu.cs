...
{
    public partial class Main2WTransformerMenu : Form
    {
        List<WindingTypeVectorHVLV> windingTypeVectorHVLVList;
        List<WindingTypeVectorHVTV> windingTypeVectorHVTVList;
        C2WTransformer transformer;
        TransformersCondition transformersCondition;
        public Main2WTransformerMenu(C2WTransformer input)
        {
            InitializeComponent();

            C2WTransformerBL transformerBL;
            transformerBL = new C2WTransformerBL();
            ...
            //comboBoxCoreConstruction.DataSource = Enum.GetValues(typeof(CoreConstruction2W)).Cast<CoreConstruction2W>().ToList();
            //comboBoxCoreConstruction.SelectedItem = CoreConstruction2W.ThreeLeggedCoreType;
            ...
            //comboBox1.DataSource = Enum.GetValues(typeof(InsulationTypesEnum)).Cast<InsulationTypesEnum>().ToList();
            //comboBox1.SelectedItem = InsulationTypesEnum.LiquidFill;
            //comboBox2.DataSource = Enum.GetValues(typeof(CoolingClassEnum)).Cast<CoolingClassEnum>().ToList();
            ...
            //coolType1Box.Name = comboBox2.SelectedItem.ToString().Split('-')[0];
            //coolType2Box.Name = comboBox2.SelectedItem.ToString().Split('-')[1];
            //coolType3Box.Name = comboBox2.SelectedItem.ToString().Split('-')[2];
        ...
        public void LoadData()
        {
            ...
            subStationNumberTXT.Value = transformer.substation.Substation_Number;
            subStationNameTXT.Text = transformer.substation.Substation_Name;
            ...
            // TAB 3PART 1 [INTER COILs IMPEDANCES]
            ...
            //TAB3 PART 2 [STARTING INTER COILs IMPEDANCES]
            ...
            //TAB 3 PART 3
            MagbetizConductace.Text = transformer.impedances.Magnetization_Conductance.ToString();
            MagbetizSusceptance.Text = transformer.impedances.Magnetization_Susceptance.ToString();
            ...
            noload.Checked = transformer.no_load;
            magnetizationcheckbox.Checked = transformer.magnetization;
            //TAB3 PART 4
            ...
            PrimHV_KV_Box.Text = (Double.Parse(PrimHVNominalBox.Text, System.Globalization.CultureInfo.InvariantCulture) * Double.Parse(RatedVoltageHV.Text, System.Globalization.CultureInfo.InvariantCulture)).ToString();
            Sec_LV_KV_Box.Text = (Double.Parse(SecLVNominalBox.Text, System.Globalization.CultureInfo.InvariantCulture) * Double.Parse(RatedVoltageLV.Text, System.Globalization.CultureInfo.InvariantCulture)).ToString();
            ...
            ShortTWinBox.Text = transformer.tempLimits.ShortTermWinding.ToString();
            ShortTPartsBox.Text = transformer.tempLimits.ShortTermParts.ToString();
            LongTWinBox.Text = transformer.tempLimits.LongTermWinding.ToString();
            LongTPartsBox.Text = transformer.tempLimits.LongTermParts.ToString();
        ...
        {
            TransformerCondition2W transformersCondition = new TransformerCondition2W();
        ...
        {
            LTC_TapDependentImpedance2W tapDependentImpedance = new LTC_TapDependentImpedance2W(transformer, mode);
        ...
        private void magnetizationcheckbox_CheckedChanged(object sender, EventArgs e)
        {
            if (magnetizationcheckbox.Checked == true)
            {
                ...
                MagbetizConductace.Enabled = true;
                MagbetizSusceptance.Enabled = true;
            ...
            else if (noload.Checked == true)
            {
                ...
                MagbetizConductace.Enabled = false;
                MagbetizSusceptance.Enabled = false;
        ...
        private void subStationNameTXT_TextChanged(object sender, EventArgs e)
        {
            if (subStationNameTXT.Text.ToUpper().Contains("Unknown"))
            {
                subStationNumberTXT.Value = 0;
        ...
        //Disable Topology tab's area and length ratio input boxes if the transforemer core is a ThreeLeggedCore
        private void comboBoxCoreConstruction_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (comboBoxCoreConstruction.Text.Contains(CoreConstruction2W.ThreeLeggedCoreType.GetDisplayName()))
            {
                ReturnAR.Enabled = false;
                ReturnLR.Enabled = false;
        ...
        public Boolean save()
        {
            try
            {
                ...
                if (transformer.PrimaryBUS.area != null)
                {
                ...
                /// TAB General
                if (transformer.SecondaryBUS.area != null)
                {
                    /*   transformer.PrimaryBUS.BusName = busNameHV.Text;
                       transformer.PrimaryBUS.BusNumber= long.Parse(busNumberHV.Text );
   ...
   */
                ...
                transformer.impedances.Magnetization_Conductance = double.Parse(MagbetizConductace.Text);
                transformer.impedances.Magnetization_Susceptance = double.Parse(MagbetizSusceptance.Text);
                ...
                transformer.no_load = noload.Checked;
                transformer.magnetization = magnetizationcheckbox.Checked;
        ...
        {
            if (Main_Menu.GetClickedDiagramNode() is C2WTShape)
            {
                var item = ((Main_Menu.GetCurrentDiagram().SelectedItems as SelectorViewModel).Nodes as IEnumerable<object>).FirstOrDefault();
                if (item != null && item is C2WTShape)
                {
                    int i = 0;
                    foreach (AnnotationEditorViewModel img in (item as C2WTShape).Annotations as IEnumerable<object>)
                    {
                        if (i == 0)
                        {
                            img.Content = (Main_Menu.GetClickedDiagramNode() as C2WTShape).getImageList()[comboBoxHV.SelectedIndex];
                        }
                        else
                        {
                            img.Content = (Main_Menu.GetClickedDiagramNode() as C2WTShape).getImageList()[comboBoxLV.SelectedIndex];
                        }
                        i++;
                    };
                }
            ...
            {
                transformer.transformer_pu.R1_2_pu = Convert_w2ctransformer_pu.calculateR_puPsc(transformer);
                transformer.transformer_pu.X1_2_pu = Convert_w2ctransformer_pu.calculateX_puPsc(transformer);
            ...
            {
                transformer.transformer_pu.R1_2_pu = Convert_w2ctransformer_pu.calculateR_puX1R1(transformer);
                transformer.transformer_pu.X1_2_pu = Convert_w2ctransformer_pu.calculateX_puX1R1(transformer);
